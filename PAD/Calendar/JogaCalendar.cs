...
namespace PAD
{
    public class JogaCalendar: Calendar
    {
        public int Type { get; set; }
        public EJoga JogaCode { get; set; }
        ...
        public JogaCalendar(EJoga jCode, DateTime startDate, DateTime endDate, DayOfWeek vara, ENakshatra nakshatraCode, int tithiId)
        {
            int type = 0;
            if (jCode == EJoga.DWIPUSHKAR  || jCode == EJoga.TRIPUSHKAR || jCode == EJoga.AMRITASIDDHA || jCode == EJoga.SARVARTHA || jCode == EJoga.SIDDHA)
            {
                type = 1;
            }
            ...
            DateStart = startDate;
            DateEnd = endDate;
            ColorCode = (EColor)(CacheLoad._jogaList.Where(i => i.Code.Equals(jCode.ToString())).FirstOrDefault()?.ColorId ?? 0);
            Type = type;
            JogaCode = jCode;
            Vara = vara;
        ...
        {
            return CacheLoad._jogaDescList.Where(i => i.JogaId == (int)JogaCode && i.LanguageCode.Equals(langCode.ToString())).FirstOrDefault()?.ShortName ?? string.Empty;
        }
        ...
        public List<JogaCalendar> CheckDvipushkarJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> dvipushkarJoga = new List<JogaCalendar>();
                ...
                if ((dateN != null && dateN.Count > 0) && (dateT != null && dateT.Count > 0))
                {
                    foreach (NakshatraCalendar nc in dateN)
                    {
                        foreach (TithiCalendar tc in dateT)
                        {
                            ...
                            if (overlap)
                            {
                                dvipushkarJoga.Add(new JogaCalendar(
                                                            EJoga.DWIPUSHKAR,
        ...
        public List<JogaCalendar> CheckTripushkarJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> tripushkarJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<NakshatraCalendar> nakshatraList = new List<NakshatraCalendar>();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
                ...
                if ((dateN != null && dateN.Count > 0) && (dateT != null && dateT.Count > 0))
                {
                    foreach (NakshatraCalendar nc in dateN)
                    {
                        foreach (TithiCalendar tc in dateT)
                        {
                            bool overlap = nc.DateStart < tc.DateEnd && tc.DateStart < nc.DateEnd;
                            if (overlap)
                                tripushkarJoga.Add(new JogaCalendar(
                                                            EJoga.TRIPUSHKAR, 
                                                            CalculateJogaStartDate(startPeriod.Value, endPeriod.Value, nc.DateStart, tc.DateStart), 
                                                            CalculateJogaEndDate(startPeriod.Value, endPeriod.Value, nc.DateEnd, tc.DateEnd), 
                                                            jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckAmritaSiddhaJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> amritaSiddhaJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<NakshatraCalendar> nakshatraList = new List<NakshatraCalendar>();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
                ...
                if ((dateN != null && dateN.Count > 0) && (dateT != null && dateT.Count > 0))
                {
                    foreach (NakshatraCalendar nc in dateN)
                    {
                        foreach (TithiCalendar tc in dateT)
                        {
                            ...
                            bool overlap = nc.DateStart < tc.DateEnd && tc.DateStart < nc.DateEnd;
                            if (overlap)
                                amritaSiddhaJoga.Add(new JogaCalendar(
                                                            EJoga.AMRITASIDDHA, 
                                                            CalculateJogaStartDate(startPeriod.Value, endPeriod.Value, nc.DateStart, tc.DateStart), 
                                                            CalculateJogaEndDate(startPeriod.Value, endPeriod.Value, nc.DateEnd, tc.DateEnd), 
                                                            jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckSarvarthaSiddhaJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> sarvarthaSiddhaJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<NakshatraCalendar> nakshatraList = new List<NakshatraCalendar>();
            ...
            if (nakshatraList.Count > 0)
            {
                ...
                {
                    foreach (NakshatraCalendar nc in dateN)
                        sarvarthaSiddhaJoga.Add(new JogaCalendar(
                                                        EJoga.SARVARTHA,
        ...
        public List<JogaCalendar> CheckSiddhaJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> siddhaJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
            ...
            if (tithiList.Count > 0)
            {
                ...
                {
                    foreach (TithiCalendar tc in dateT)
                        siddhaJoga.Add(new JogaCalendar(
                                                EJoga.SIDDHA, 
        ...
        public List<JogaCalendar> CheckLargeSiddhaJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> largeSiddhaJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<NakshatraCalendar> nakshatraList = new List<NakshatraCalendar>();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
                ...
                if ((dateN != null && dateN.Count > 0) && (dateT != null && dateT.Count > 0))
                {
                    foreach (NakshatraCalendar nc in dateN)
                    {
                        foreach (TithiCalendar tc in dateT)
                        {
                            ...
                            bool overlap = nc.DateStart < tc.DateEnd && tc.DateStart < nc.DateEnd;
                            if (overlap)
                                largeSiddhaJoga.Add(new JogaCalendar(
                                                            EJoga.SIDDHA, 
                                                            CalculateJogaStartDate(startPeriod.Value, endPeriod.Value, nc.DateStart, tc.DateStart), 
                                                            CalculateJogaEndDate(startPeriod.Value, endPeriod.Value, nc.DateEnd, tc.DateEnd), 
                                                            jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckMritjuJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> mritjuJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
            ...
            if (tithiList.Count > 0)
            {
                ...
                {
                    foreach (TithiCalendar tc in dateT)
                        mritjuJoga.Add(new JogaCalendar(
                                                EJoga.MRITYU, 
                                                CalculateTithiJogaStartDate(startPeriod.Value, endPeriod.Value, tc.DateStart), 
                                                CalculateTithiJogaEndDate(startPeriod.Value, endPeriod.Value, tc.DateEnd), 
                                                jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckAdhamJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> adhamJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
            ...
            if (tithiList.Count > 0)
            {
                ...
                {
                    foreach (TithiCalendar tc in dateT)
                        adhamJoga.Add(new JogaCalendar(
                                                EJoga.ADHAM, 
                                                CalculateTithiJogaStartDate(startPeriod.Value, endPeriod.Value, tc.DateStart), 
                                                CalculateTithiJogaEndDate(startPeriod.Value, endPeriod.Value, tc.DateEnd), 
                                                jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckYamaghataJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> yamaghataJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<NakshatraCalendar> nakshatraList = new List<NakshatraCalendar>();
            ...
            if (nakshatraList.Count > 0)
            {
                ...
                {
                    foreach (NakshatraCalendar nc in dateN)
                        yamaghataJoga.Add(new JogaCalendar(
                                                    EJoga.YAMAGHATA, 
                                                    CalculateNakshatraJogaStartDate(startPeriod.Value, endPeriod.Value, nc.DateStart), 
                                                    CalculateNakshatraJogaEndDate(startPeriod.Value, endPeriod.Value, nc.DateEnd), 
                                                    jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckDagdhaJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> dagdhaJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<NakshatraCalendar> nakshatraList = new List<NakshatraCalendar>();
            ...
            if (nakshatraList.Count > 0)
            {
                ...
                {
                    foreach (NakshatraCalendar nc in dateN)
                        dagdhaJoga.Add(new JogaCalendar(
                                                EJoga.DAGDHA, 
                                                CalculateNakshatraJogaStartDate(startPeriod.Value, endPeriod.Value, nc.DateStart), 
                                                CalculateNakshatraJogaEndDate(startPeriod.Value, endPeriod.Value, nc.DateEnd), 
                                                jogaPeriod[0].Date.DayOfWeek, 
        ...
        public List<JogaCalendar> CheckUnfarobaleJoga(DateTime? startPeriod, DateTime? endPeriod, List<Day> daysList)
        {
            List<JogaCalendar> unfarobaleJoga = new List<JogaCalendar>();
            List<Day> jogaPeriod = daysList.Where(i => i.Date >= startPeriod.Value.Date && i.Date <= endPeriod.Value.Date).ToList();
            List<TithiCalendar> tithiList = new List<TithiCalendar>();
            ...
            if (tithiList.Count > 0)
            {
                ...
                {
                    foreach (TithiCalendar tc in dateT)
                        unfarobaleJoga.Add(new JogaCalendar(
                                                EJoga.UNFAVORABLE, 
                                                CalculateTithiJogaStartDate(startPeriod.Value, endPeriod.Value, tc.DateStart), 
                                                CalculateTithiJogaEndDate(startPeriod.Value, endPeriod.Value, tc.DateEnd), 
                                                jogaPeriod[0].Date.DayOfWeek, 
