    ...
    public abstract class AbstractRenderer : IDisposable
    {
        ...
        public virtual Font HourFont
        {
            get
            {
                if (hourFont == null)
                {
                    hourFont = new Font(BaseFont.FontFamily, 10, FontStyle.Regular);
                }
        ...
        public virtual Font MinuteFont
        {
            get
            {
                if (minuteFont == null)
                {
                    minuteFont = new Font(BaseFont.FontFamily, 6, FontStyle.Regular);
                }
        ...
        public virtual Font SunFont
        {
            get
            {
                if (sunFont == null)
                {
                    sunFont = new Font(BaseFont.FontFamily, 8, FontStyle.Regular);
                }
        ...
        public abstract void DrawHourLabel(Graphics g, Rectangle rect, int hour, bool ampm, string cultureCode);

        public abstract void DrawSunLabel(Graphics g, Rectangle rect, bool isSunRise, DateTime sunDate, Color color, string cultureCode);

        public abstract void DrawMinuteLine(Graphics g, Rectangle rect);

        public abstract void DrawMinuteLine(Graphics g, int posX, int posY, int width, Color color);

        public abstract void DrawDayHeader(Graphics g, Rectangle rect, DateTime date, int day, string linesName, string cultureCode);
        ...
        public virtual void DrawDayGripper(Graphics g, Rectangle rect, int gripWidth)
        {
            ...
            using (Brush m_Brush = new SolidBrush(Color.White))
                g.FillRectangle(m_Brush, rect.Left, rect.Top - 1, 0.01f, rect.Height);

            using (Pen m_Pen = new Pen(Color.Black))
                g.DrawRectangle(m_Pen, rect.Left, rect.Top - 1, 0.01f, rect.Height);
        }
